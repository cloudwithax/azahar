...
void JitShader::Compile_SanitizedMul(QReg src1, QReg src2, QReg scratch0) {
    if (!Settings::values.shaders_accurate_mul.GetValue()) {
...
void JitShader::Compile_RCP(Instruction instr) {
    ...
    // Use FRECPE + FRECPS (one Newton-Raphson iteration) for ~1-2 ULP accuracy.
    // This is 3-5x faster than scalar FDIV on Cortex-A55/A72.
    FRECPE(SRC1.toS(), SRC1.toS());
    FRECPS(VSCRATCH0.toS(), SRC1.toS(), SRC1.toS());
...
void JitShader::Compile_RSQ(Instruction instr) {
    ...
    // Use FRSQRTE + FRSQRTS (one Newton-Raphson iteration) for ~1-2 ULP accuracy.
    // This is 3-5x faster than scalar FSQRT+FDIV on Cortex-A55/A72.
    FRSQRTE(SRC1.toS(), SRC1.toS());
    FRSQRTS(VSCRATCH0.toS(), SRC1.toS(), SRC1.toS());
    FMUL(SRC1.toS(), SRC1.toS(), VSCRATCH0.toS());
